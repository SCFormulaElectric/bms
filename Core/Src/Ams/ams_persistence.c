...
#define AMS_FLASH_RECORD_VERSION  4U
...
/* Version 3 did not contain a telemetry layout. It remains readable so a
 * firmware update does not discard calibrated AMS limits and SOC. */
typedef struct {
    uint32_t magic;
    uint16_t version;
    uint16_t record_bytes;
...
static uint8_t record_valid(const ams_flash_record_t *record)
{
    if (record->magic != AMS_FLASH_RECORD_MAGIC ||
        ...
        bms_summary_config_is_valid(&record->summary_config) == 0U) {
        return 0U;
    }
...
static uint8_t record_v3_valid(const ams_flash_record_v3_t *record)
{
    return (record->magic == AMS_FLASH_RECORD_MAGIC &&
        record->version == 3U && record->record_bytes == sizeof(*record) &&
        record->soc_permille <= 1000U &&
        ams_config_is_valid(&record->config) != 0U &&
        crc32((const uint8_t *)record,
            offsetof(ams_flash_record_v3_t, crc32)) == record->crc32) ?
        1U : 0U;
}

static const ams_flash_record_v3_t *latest_v3_record(void)
{
    const ams_flash_record_v3_t *latest = NULL;
    uint32_t address;
    const uint32_t stride =
        (uint32_t)((sizeof(ams_flash_record_v3_t) + 3U) & ~3U);
    const uint32_t end = AMS_FLASH_SECTOR_ADDRESS + AMS_FLASH_SECTOR_BYTES;

    for (address = AMS_FLASH_SECTOR_ADDRESS;
        address + sizeof(ams_flash_record_v3_t) <= end; address += stride) {
        const ams_flash_record_v3_t *candidate =
            (const ams_flash_record_v3_t *)address;
        ...
        if (record_v3_valid(candidate) != 0U &&
            (latest == NULL ||
    ...
    uint32_t *sequence)
{
    ...
    if (config == NULL || summary_config == NULL || soc_permille == NULL) {
        return AMS_PERSISTENCE_INVALID;
    }
    record = latest_record(NULL);
    if (record == NULL) {
        const ams_flash_record_v3_t *legacy = latest_v3_record();
        if (legacy == NULL) {
            return AMS_PERSISTENCE_NOT_FOUND;
        }
        *config = legacy->config;
        *soc_permille = legacy->soc_permille;
        if (bms_summary_config_is_valid(summary_config) == 0U) {
            bms_summary_config_load_defaults(summary_config);
        }
        if (sequence != NULL) {
    ...
    *summary_config = record->summary_config;
    ...
    uint8_t discharge_output_active, uint8_t charge_output_active)
{
    ...
    if (ams_config_is_valid(config) == 0U ||
        bms_summary_config_is_valid(summary_config) == 0U ||
        soc_permille > 1000U) {
        return AMS_PERSISTENCE_INVALID;
    }
    ...
    record.summary_config = *summary_config;
